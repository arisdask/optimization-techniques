...
% info: This script only has the solution of Problem 3 (Fibonacci Method)
...
for i = 1:1:sampleSize
    l = lList(i);
    [~, ~, n1] = fibonacciMethod(matlabFunction(f1), a, b, l);
    [~, ~, n2] = fibonacciMethod(matlabFunction(f2), a, b, l);
    [~, ~, n3] = fibonacciMethod(matlabFunction(f3), a, b, l);
...
ylabel('Total f_i(x) calculations n+2', 'FontSize', 15);
xlabel('Tolerance l', 'FontSize', 15);
title('(2+n) - l diagram', 'FontSize', 17);
...
for p = 1:1:length(funcArray)
    figure('Name', ['Fibonacci Method Bounds in f', num2str(p), '(x)']);

    for i = 1:1:length(lValues)
        l = lValues(i);
        [ak, bk, n] = fibonacciMethod(funcArray{p}, a, b, l);
