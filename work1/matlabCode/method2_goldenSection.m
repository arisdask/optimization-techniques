...
% info: This script only has the solution of Problem 2 (Golden Section)
...
for i = 1:1:sampleSize
    l = lList(i);
    [~, ~, n1] = goldenSection(matlabFunction(f1), a, b, l);
    [~, ~, n2] = goldenSection(matlabFunction(f2), a, b, l);
    [~, ~, n3] = goldenSection(matlabFunction(f3), a, b, l);
...
for p = 1:1:length(funcArray)
    figure('Name', ['Golden Section Method Bounds in f', num2str(p), '(x)']);

    for i = 1:1:length(lValues)
        l = lValues(i);
        [ak, bk, n] = goldenSection(funcArray{p}, a, b, l);
